...
- Tested with Chromium Pixel emulation (SwiftShader), portrait 412×915 and landscape 915×360.

**Round 2 (Alan's Pixel 10, inside the Claude Android app's artifact viewer, ~640×280 CSS px sideways):**
- The app's WebView has **no Web Speech API**. `skipToFight` called `speechSynthesis?.cancel()`, which throws a ReferenceError on an undeclared global. All speech now goes through `sound.hush()` / `sound.say()`, which do nothing when the API is missing, so the film plays with subtitles only there. (A wordless "murmur" fallback was tried and Alan rejected it on 2026-09-26: keep the original spoken narration.)
